...
const AnimatedCircle = Animated.createAnimatedComponent<Partial<CircleProps>>(Circle);
...
export const CircleLoadingIndicator: React.FC<ICircleLoadingIndicator> =
  memo<ICircleLoadingIndicator>(
    ({
      ...
      | ICircleLoadingIndicator): React.JSX.Element & React.ReactNode & React.ReactElement => {
      ...
      const animatedProps1 = useAnimatedProps<Required<Partial<Pick<CircleProps, 'cy'>>>>(() => ({
        cy: 12 - progress1.value * jumpHeight,
      }));
      const animatedProps2 = useAnimatedProps<Required<Partial<Pick<CircleProps, 'cy'>>>>(() => ({
        cy: 12 - progress2.value * jumpHeight,
      }));
      const animatedProps3 = useAnimatedProps<Required<Partial<Pick<CircleProps, 'cy'>>>>(() => ({
