...
const OtpItem: FC<IOtpItem> & FunctionComponent<IOtpItem> = ({
  index,
}: IOtpItem): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => {
  const {
    inputRef,
    onPress,
    otpValue,
  ...
  const animatedInputStyle = useAnimatedStyle<
    Pick<ViewStyle, 'borderWidth' | 'borderColor' | 'backgroundColor' | 'transform'>
  >(() => {
...
export const OtpInput: FC<IOtpInput> & FunctionComponent<IOtpInput> = memo<IOtpInput>(
  ({
    otpCount = 6,
    containerStyle = {},
    otpInputStyle = {},
    ...
    const onFocusNext = <V extends string, I extends number>(value: V, index: I) => {
    ...
    const onFocusPrevious = <K extends string, I extends number>(key: K, index: I) => {
    ...
    const animatedContainerStyle = useAnimatedStyle<Pick<ViewStyle, 'opacity' | 'transform'>>(
      () => ({
    ...
    useEffect(() => {
      ...
      if (otpValue && otpValue.join('').length === otpCount && onInputFinished) {
        if (!error) {
